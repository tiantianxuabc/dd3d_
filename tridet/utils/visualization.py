...
from tridet.utils.wandb import flatten_dict
...
def mosaic(items, scale=1.0, pad=3, grid_width=None):
    ...
    grid_width = 2 #grid_width if grid_width else np.ceil(np.sqrt(N)).astype(int)
    grid_height = N // 2 + 1 if N % 2 != 0 else  N / 2 #np.ceil(N * 1. / grid_width).astype(np.int)
